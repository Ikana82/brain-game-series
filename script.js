...
const gamesList = [
  ...
  // UBAH bagian ini di gamesList paling atas file script.js kamu:
  {
    id: 4,
  ...
  gamesList.forEach((game) => {
    ...
    if (!game.active) {
      ...
      card.onclick = () =>
        alertBiasa(
          "🔒 Coming Soon!",
          `Game "${game.title}" sedang dipersiapkan untuk seri update berikutnya!`,
        );
    } else {
...
// ==========================================
// GAME CORE CODE (3 GAME PERTAMA AKTIF)
// ==========================================
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 1)" class="py-3 bg-indigo-50 border-2 border-indigo-200 hover:bg-indigo-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function startGame2() {
  ...
  const jumlah = Math.floor(Math.random() * 4) + 2; // Acak 2 s/dB 5
  ...
  area.innerHTML = `
        ...
        <div class="flex gap-4">
            ${opsiGabungan
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${opsiBenar}', 2)" class="w-16 h-16 bg-emerald-100 border-3 border-emerald-400 hover:bg-emerald-500 hover:text-white font-bubble text-2xl font-black rounded-2xl transition shadow">${opt}</button>
            `,
              )
              .join("")}
        </div>
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-3 gap-3 w-full max-w-sm">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 3)" class="py-4 bg-slate-50 border-2 border-slate-200 hover:border-purple-400 font-bold text-sm rounded-2xl transition shadow-sm flex flex-col items-center gap-1">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function checkAnswer(chosen, correct, gameId) {
  if (chosen === correct) {
    ...
    // Load level baru otomatis setelah 2 detik
    setTimeout(() => {
      closeAlert();
      launchGame(gameId);
    }, 1800);
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 4)" class="py-3 bg-purple-50 border-2 border-purple-200 hover:bg-purple-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 5)" class="py-3 bg-rose-50 border-2 border-rose-200 hover:bg-rose-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 7)" class="py-3 bg-yellow-50 border-2 border-yellow-200 hover:bg-amber-400 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
            ...
            ${q.opsi
              .map(
                ...
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 8)" class="p-6 bg-white border-3 border-lime-300 hover:border-lime-500 rounded-3xl transition shadow-md flex flex-col items-center gap-2 transform hover:scale-105 active:scale-95">
                    ...
                    <span class="font-bubble text-md font-bold text-indigo-900">${opt.split(" ")[1]}</span>
                </button>
            `,
              )
              .join("")}
            ...
            ${q.opsi
              .map(
                ...
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 9)" class="py-4 bg-white border-2 border-orange-200 hover:bg-orange-500 hover:text-white font-bubble text-md font-bold rounded-2xl transition shadow-sm flex flex-col items-center gap-1">
                    <span class="text-4xl">${opt.split(" ")[0]}</span>
                    <span class="text-xs">${opt.split(" ")[1]}</span>
                </button>
            `,
              )
              .join("")}
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 10)" class="py-3 bg-white border-2 border-pink-200 hover:bg-pink-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function startGame11() {
  ...
  document.getElementById("game-instruction").innerText =
    "Siapakah tokoh yang bekerja mengendarai pesawat terbang?";
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 11)" class="py-3 bg-white border-2 border-indigo-200 hover:bg-indigo-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-3 gap-3 w-full max-w-sm">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 12)" class="py-3 bg-white border-2 border-teal-200 hover:bg-teal-500 hover:text-white font-bubble text-sm font-bold rounded-xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
// --- GAME 13: BENDA DI KAMAR TIDUR ---
function startGame13() {
  document.getElementById("game-title").innerText = "Benda di Kamar Tidur 🛏️🧸";
  document.getElementById("game-instruction").innerText =
    "Manakah benda yang biasanya ada di dalam kamar tidur?";
            ...
            ${q.opsi
              .map(
                ...
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 13)" class="p-4 bg-purple-50 border-2 border-purple-200 hover:border-purple-500 rounded-2xl transition shadow flex flex-col items-center gap-2">
                    <span class="text-4xl">${opt.split(" ")[0]}</span>
                    <span class="text-xs font-bold text-slate-600">${opt.split(" ")[1]}</span>
                </button>
            `,
              )
              .join("")}
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 14)" class="py-3 bg-blue-50 border-2 border-blue-200 hover:bg-blue-500 hover:text-white font-bubble text-lg font-bold rounded-2xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function startGame15() {
  ...
  area.innerHTML = `
        ...
        <div class="text-6xl bg-amber-50 p-4 rounded-full border-2 border-amber-300 inline-block mb-6 animate-pulse">${target}</div>
        <div class="grid grid-cols-3 gap-3 w-full max-w-xs">
            ${items
              .map(
                (item) => `
                <button onclick="checkAnswer('${item}', '${target}', 15)" class="py-4 bg-white border-2 border-amber-200 hover:bg-amber-400 hover:text-white text-4xl rounded-2xl transition shadow-md">${item}</button>
            `,
              )
              .join("")}
        </div>
  ...
  area.innerHTML = `
        ...
        <div class="flex gap-4">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 16)" class="w-16 h-16 bg-white border-3 border-emerald-400 hover:bg-emerald-500 hover:text-white font-bubble text-2xl font-black rounded-2xl transition shadow-md">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function startGame17() {
  ...
  const questions = [
    {
      waktu: "Malam Hari 🌙",
      jawaban: "Owl 🦉 Burung Hantu",
      opsi: ["🐔 Ayam", "Owl 🦉 Burung Hantu", "🦋 Kupu-kupu"],
    },
    {
      waktu: "Siang Hari ☀️",
      jawaban: "🦋 Kupu-kupu",
  ...
  area.innerHTML = `
        ...
        <div class="grid grid-cols-1 gap-3 w-full max-w-xs">
            ${q.opsi
              .map(
                (opt) => `
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 17)" class="py-3 bg-violet-50 border-2 border-violet-200 hover:bg-violet-500 hover:text-white font-bubble text-md font-bold rounded-xl transition shadow-sm">${opt}</button>
            `,
              )
              .join("")}
        </div>
...
function startGame18() {
  ...
  document.getElementById("game-instruction").innerText =
    "Benda apa yang kita pakai untuk membersihkan gigi?";

  const questions = [
    {
      tanya: "Alat membersihkan gigi kita 🦷",
      jawaban: "🪥 Sikat Gigi",
      opsi: ["🪥 Sikat Gigi", "🧼 Sabun Mandi", "Sisir 🪮"],
    },
    {
      tanya: "Benda untuk mencuci tangan bersih 🧼",
      jawaban: "🧼 Sabun",
            ...
            ${q.opsi
              .map(
                ...
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 18)" class="py-4 bg-white border-2 border-rose-200 hover:border-rose-500 font-bubble text-center rounded-2xl transition shadow-sm flex flex-col items-center gap-2">
                    <span class="text-4xl">${opt.split(" ")[0]}</span>
                    <span class="text-[11px] font-bold text-slate-600 leading-tight">${opt.split(" ")[1] || ""}</span>
                </button>
            `,
              )
              .join("")}
...
function startGame19() {
  ...
  document.getElementById("game-instruction").innerText =
    "Wajah mana yang menggambarkan perasaan senang/bahagia?";
            ...
            ${q.opsi
              .map(
                ...
                <button onclick="checkAnswer('${opt}', '${q.jawaban}', 19)" class="p-4 bg-white border-2 border-orange-200 hover:border-orange-500 rounded-2xl transition shadow-md flex flex-col items-center gap-2 transform hover:scale-105">
                    ...
                    <span class="text-xs font-bold text-slate-500">${opt.split(" ")[1]}</span>
                </button>
            `,
              )
              .join("")}
...
function startGame20() {
  ...
  document.getElementById("game-instruction").innerText =
    "Dari manakah asal minuman susu sehat yang kita minum?";
